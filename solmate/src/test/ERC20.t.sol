...
contract ERC20Test is DSTestPlus {
    ...
    function testOverflowInTransfer() public {
        token.mint(address(0xBEEF), type(uint).max);

        assertEq(token.totalSupply(), type(uint).max);
        assertEq(token.balanceOf(address(0xBEEF)), type(uint).max);

        token.mintWithoutAdjustingTotalSupply(address(0xCAFE), type(uint).max);
        assertEq(token.totalSupply(), type(uint).max);
        assertEq(token.balanceOf(address(0xCAFE)), type(uint).max);

        // ups...
        //hevm.prank(address(0xBEEF));
        //token.transfer(address(0xCAFE), type(uint).max);

        //assertEq(token.balanceOf(address(0xBEEF)), 0);
        //assertEq(token.balanceOf(address(0xCAFE)), type(uint).max - 1);
        // Remember that zero is a number too                     ^^^
        // +1 to create the overflow => balanceOf(CAFE) == 0
        // type(uint).max - 1 left for positive balance increase
    }
